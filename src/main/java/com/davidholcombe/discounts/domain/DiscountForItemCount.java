...
@Getter
@Builder
public class DiscountForItemCount implements Discount {
    ...
    public BigDecimal calculateDiscountedTotalCost(final Map<Item, Long> items) {
        ...
        final ImmutableMap<Item, Long> itemsValidForDiscount = items.entrySet().stream()
                .filter(entry -> entry.getKey().getId() == itemId && entry.getValue() >= quantity)
    ...
    public static Discount from(final DiscountEntity entity) {
        ...
        return DiscountForItemCount.builder()
                .code(entity.getCode())
                .type(entity.getType())
